...
Сценарий: Перебор всех строк таблицы с вложенной проверкой
	...
	И Пока выражение встроенного языка "Контекст.СлужебнаяПеременная < 3" истинно тогда
			...
			И     в ТЧ "ТабличнаяЧасть2" в поле "Реквизит число" я ввожу текст 1
	...
	И для каждой строки таблицы "ТабличнаяЧасть1" я выполняю
			...
			И     в ТЧ "ТабличнаяЧасть1" в поле "Реквизит число" я ввожу текст 1
			И     В форме "Справочник1 (создание) *" в ТЧ "ТабличнаяЧасть1" я завершаю редактирование строки
			И     Я запоминаю значение выражения "Контекст.СлужебнаяПеременная+1" в переменную "СлужебнаяПеременная"
	...
	И     таблица "ТабличнаяЧасть2" стала равной по шаблону:
		|  'N' | 'Реквизит число' |
		|  '1' | '5*00'           |
		|  '2' | '5*00'           |
		|  '3' | '5*00'           |

Сценарий: Установка глобальной переменной
	Когда Я запоминаю значение выражения "1" в переменную "ГлобальныйФлагТогоЧтоЭтотШагВыполнялся" глобально

Сценарий: Проверка глобальной переменной
	Тогда выражение внутреннего языка "КонтекстСохраняемый.ГлобальныйФлагТогоЧтоЭтотШагВыполнялся = 1" Истинно
